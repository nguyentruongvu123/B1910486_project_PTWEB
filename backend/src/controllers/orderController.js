...
const orderController = {
  ...
  listOrder: async (req, res) => {
    try {
      const order = await ORDER.find().populate("book_id").populate("prod_id_list.id_prod").sort({ status_order: 1 });
      if (!order) {
        return res.json({
          success: false,
  ...
  getOrder: async (req, res) => {
    try {
      ...
      const get_order = await ORDER.findOne({ _id: order_id }).populate("prod_id_list.id_prod");
  ...
  updateStatusOrder: async (req, res) => {
    try {
      ...
      const update = await ORDER.findOneAndUpdate({ _id: idOrder }, { status_order: status });
      if (status == 4) {
  ...
  createOrder: async (req, res) => {
    try {
      const { name_order, quantity_order, price_order, prod_id_list, book_id } = req.body;
