<template>
  <div class="table-admin-custom">
      ...
      <tbody class="body-table-custom" v-show="!loadingTable">
        ...
        <tr v-for="(item, idx) in content" :key="idx">
          ...
          <td>
            <div class="">
              <p
                class="item-import-prod"
                v-for="(list, idx) in item.list_prod"
                :key="idx"
              >
                {{ list.name_prod }}
              </p>
    ...
    >
      ...
      <p>Xác nhận xoá đơn nhập hàng này ?</p>
    </a-modal>
  </div>
</template>
...
export default {
  ...
  // mounted() {
  //   console.log("get data content table ----", this.content);
  // },
  watch: {},
  methods: {
    handleEdit(event) {
      this.$router.push({
        path: "/admin/import-prod/update/" + event,
      });
    },
    ...
    async handleDel(event) {
      ...
      const response = await this.$axios.delete(
        url + "/import/delete-import/delete/" + event
      );
